...
### `song_versions` table (GraceTracks-only, migration `20260612000000`)

A song can have multiple named stem sets ("versions", e.g. `AGMC2026`, `HQ`, `GA2024`). A song with **no rows** here has a single implicit **"Original"** version whose stems sit at the legacy R2 path — no backfill needed.

| Column | Type | Description |
|---|---|---|
| `id` | `uuid` | Primary key |
| `song_slug` | `text` | FK → `songs.slug` (`ON UPDATE/DELETE CASCADE`) |
| `version_slug` | `text` | URL/R2-safe id (`[a-z0-9_-]{1,64}`; `original` is reserved) |
| `label` | `text` | Display name (e.g. `AGMC2026`) |
| `is_default` | `boolean` | ≤1 per song (partial unique index); no flagged row = Original is default |
| `created_at` | `timestamptz` | Order in version menus |

RLS: public SELECT only for versions of visible songs; editor+ full access (role from `public.users`). Client helpers live in `src/lib/versions.js`.
...
```
{VITE_R2_PUBLIC_URL}/tracks/{stem_slug}/{stem_name}.{ext}                            # "Original" version (legacy path)
{VITE_R2_PUBLIC_URL}/tracks/{stem_slug}/versions/{version_slug}/{stem_name}.{ext}    # named versions
```

- `stem_slug` — from DB (or falls back to `slug`)
- `version_slug` — from `song_versions`; the implicit Original has no `versions/` segment, and existing objects never move
...
| `/song/:slug` | Mixer (`mixer.js`) — opens the song's default version |
| `/song/:slug?v=:version_slug` | Mixer with a specific stem version (`?v=original` = legacy stems) |
...
- Validates: slug format `[a-z0-9_-]+`, track name in allowed set, extension `.m4a` or `.wav`, optional `version` (`[a-z0-9_-]{1,64}`; absent/`original` = legacy path)
